...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  const loginAsGuest = useCallback(async () => {
    try {
      ...
      const guestUser = await authService.crearUsuarioAnonimo() // Usa tu método real
      setUser(guestUser)
    } catch (err) {
      const errorMessage =
        err instanceof Error ? err.message : 'Error al iniciar como invitado'
      setError(errorMessage)
      throw err
    } finally {
      setIsLoading(false)
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      ...
      handleRegisterSuccess as EventListener
    )
    window.addEventListener('auth:logoutSuccess', handleLogoutSuccess)
    window.addEventListener('auth:tokenExpired', handleTokenExpired)
    window.addEventListener(
      'auth:oauth2Success',
      handleOAuth2Success as EventListener
    )
